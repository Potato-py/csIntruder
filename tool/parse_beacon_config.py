...
def csBeaconParse(beacon,json,quiet,csVersion):
...
#     args = parser.parse_args()
    ...
    if os.path.isfile(beacon):
        result= cobaltstrikeConfig(beacon).parse_config(version=csVersion, quiet=quiet, as_json=json) or \
                cobaltstrikeConfig(beacon).parse_encrypted_config(version=csVersion, quiet=quiet, as_json=json)
        if result :
            return result
    elif beacon.lower().startswith('http'):
        x86_beacon_data = get_beacon_data(beacon, 'x86')
        x64_beacon_data = get_beacon_data(beacon, 'x64')
        ...
        result = cobaltstrikeConfig(BytesIO(conf_data)).parse_config(version=csVersion, quiet=quiet, as_json=json) or \
                 cobaltstrikeConfig(BytesIO(conf_data)).parse_encrypted_config(version=csVersion, quiet=quiet, as_json=json)
        if result :
            return result
    else:
